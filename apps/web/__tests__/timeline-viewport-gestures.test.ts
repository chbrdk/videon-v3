...
describe('cut timeline edit UX smoke', () => {
  it('binds snap toggle, multi-select, fit, inertia, and trim precue', () => {
    ...
    expect(css).toMatch(/track\.videon-cut-timeline__track--audio/)
    expect(css).toMatch(/track--transcript/)
    expect(css).toMatch(/min-height: 2\.25rem/)
    expect(css).not.toMatch(/track--transcript \{\s*min-height: 1\.2rem/)
