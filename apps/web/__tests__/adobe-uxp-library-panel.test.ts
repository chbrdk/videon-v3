...
describe('adobe uxp library panel contracts', () => {
  ...
  it('panel scaffold ships Premiere UXP manifest (host premierepro)', () => {
    ...
    expect(readFileSync(join(root, 'src/index.html'), 'utf8')).toContain('v0.1.13')
    ...
    expect(readFileSync(join(root, 'src/index.js'), 'utf8')).toContain("PANEL_VERSION = '0.1.13'")
    ...
    expect(readFileSync(join(root, 'src/styles.css'), 'utf8')).toContain('grid-template-columns: repeat(auto-fill')
    ...
    expect(readFileSync(join(root, 'src/panel.bundle.js'), 'utf8')).toContain('0.1.13')
