...
-- Column types match cuts / cut_exports / videon_workspaces (uuid).
...
create table if not exists cut_client_room_approvals (
  ...
  export_id uuid references cut_exports (id) on delete set null,
  approved_by_plexon_user_id uuid not null,
  guideline_id text,
