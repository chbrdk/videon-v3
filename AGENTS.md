...
4. Do not hardcode service bases, storage paths, or provider model IDs. Resolve them from `lib/paths.ts` / `lib/runtime-config.ts`. Document canonical values in `knowledge/paths.md`.
...
11. Brand compliance: VIDEON describes; Brandion owns guideline truth. See `knowledge/brand-compliance.md`. Never invent a synthetic pass.
